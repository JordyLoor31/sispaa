...
class EstudianteController extends Controller
{
    ...
    public function index(): Response
    {
        ...
        $conFaltas = $porEstudiante->filter(fn ($e) => $e->matricula_estado !== 'retirado' && $e->faltas_count > 0)->count();
        $activos = max(0, $totalEstudiantes - $retirados - $conFaltas);

        $totalFaltas = Falta::count();
        $totalJustificadas = JustificacionSolicitud::where('estado', 'aprobada')->count();
        ...
        $ultimosRegistros = DB::table('users')
            ...
            ->map(function ($u) {
                $estadoLabel = match (true) {
                    ...
                    $u->estado === 'activo' && $u->faltas_count > 0 => 'Con faltas',
                ...
                return [
                    ...
                    'lastActivity' => $u->ultima_matricula
                        ...
                        : ($u->faltas_count > 0 ? $u->faltas_count . ' falta(s) registrada(s)' : 'Sin actividad reciente'),
                ];
            });
