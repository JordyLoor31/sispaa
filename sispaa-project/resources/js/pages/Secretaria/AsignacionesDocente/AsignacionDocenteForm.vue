    ...
    <form class="space-y-5" @submit="onSubmit">
        <FormField v-slot="{ errorMessage }" name="docente_id">
            <FormItem>
                ...
                <div
                    v-if="isEditing"
                    class="flex items-center justify-between rounded-md border border-input bg-slate-50 px-3 py-2 text-sm text-slate-600 dark:bg-slate-900 dark:text-slate-400"
                >
                    <div>
                        <p class="font-medium text-slate-800 dark:text-slate-200">{{ asignacion?.docente.name }}</p>
                        <p class="text-xs text-slate-400">{{ asignacion?.docente.email }}</p>
                    </div>
                ...
                <Combobox v-else v-model="selectedDocenteObj" by="value">
                    ...
                    <ComboboxList class="w-[var(--reka-combobox-trigger-width)] min-w-[280px] rounded-lg border border-slate-100 bg-white shadow-lg dark:border-slate-900 dark:bg-slate-950">
                        ...
                        <ComboboxGroup class="max-h-60 overflow-y-auto p-1">
                            ...
                            >
                                <div>
                                    <p>{{ d.name }}</p>
                                    <p class="text-xs text-slate-400">{{ d.cedula ?? d.email }}</p>
                                </div>
                        ...
                        <ComboboxGroup class="max-h-60 overflow-y-auto p-1">
                            ...
                            >
                                <div>
                                    ...
                                    <p v-if="m.carrera" class="text-xs text-slate-400">{{ m.carrera.nombre }}</p>
                                </div>
                                <ComboboxItemIndicator><Check class="h-4 w-4 text-indigo-650" /></ComboboxItemIndicator>
                            </ComboboxItem>
                        </ComboboxGroup>
