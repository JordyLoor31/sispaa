...
import { Search, CheckCircle2, XCircle, FileText } from 'lucide-vue-next';
...
import makeExpedienteColumns, { type DocumentoRow } from './expediente-columns';
...
const breadcrumbs: BreadcrumbItemType[] = [
    ...
    { title: 'Expediente SGA', href: route('secretaria.expediente.index') },
];
...
// ── Review Modal ─────────────────────────────────────────
const reviewDoc  = ref<DocumentoRow | null>(null);
const reviewForm = useForm({ accion: '' as 'aprobar' | 'rechazar', observacion: '' });

const openReview = (doc: DocumentoRow, accion: 'aprobar' | 'rechazar') => {
    reviewDoc.value   = doc;
    reviewForm.accion = accion;
    reviewForm.observacion = '';
...
const submitReview = () => {
    ...
    reviewForm.patch(route('secretaria.expediente.review', reviewDoc.value.id), {
        preserveScroll: true,
        onSuccess: () => {
...
// ── TanStack Table ────────────────────────────────────────
const columns = makeExpedienteColumns({ onReview: openReview });
...
<template>
    <AppSidebarLayout :breadcrumbs="breadcrumbs">
        ...
        <!-- Dialog Revisión -->
        <Dialog :open="!!reviewDoc" @update:open="val => { if (!val) closeReview(); }">
            <DialogContent class="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle>
                        <span v-if="reviewForm.accion === 'aprobar'" class="flex items-center gap-2 text-emerald-600">
                            <CheckCircle2 class="h-5 w-5" /> Aprobar documento
                        </span>
                        <span v-else class="flex items-center gap-2 text-rose-600">
                            <XCircle class="h-5 w-5" /> Rechazar documento
                        </span>
                    </DialogTitle>
                    <DialogDescription v-if="reviewDoc">
                        <strong>{{ reviewDoc.tipo_documento }}</strong> — {{ reviewDoc.estudiante.name }}
                    </DialogDescription>
                </DialogHeader>
                ...
                <div class="space-y-4 py-2">
                    <!-- Archivo preview -->
                    <div
                        v-if="reviewDoc?.archivo_meta"
                        class="flex items-center gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800"
                    >
                        <FileText class="h-8 w-8 text-indigo-400 shrink-0" />
                        <div class="flex-1 min-w-0 text-xs">
                            <p class="font-semibold text-slate-700 dark:text-slate-200 truncate">{{ reviewDoc.archivo_meta.name }}</p>
                            <p class="text-slate-400">{{ reviewDoc.archivo_meta.size }}</p>
                        </div>
                        <a
                            v-if="reviewDoc.archivo_url"
                            :href="reviewDoc.archivo_url"
                            target="_blank"
                            class="shrink-0 text-xs font-semibold text-indigo-600 hover:underline"
                        >Ver PDF</a>
                    </div>

                    <!-- Observación / Motivo rechazo -->
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
                            {{ reviewForm.accion === 'rechazar' ? 'Motivo del rechazo *' : 'Observación (opcional)' }}
                        </label>
                        <textarea
                        ...
                        />
                        <p v-if="reviewForm.errors.observacion" class="text-xs text-rose-500 mt-1">
                            {{ reviewForm.errors.observacion }}
                        </p>
                    </div>
                </div>
