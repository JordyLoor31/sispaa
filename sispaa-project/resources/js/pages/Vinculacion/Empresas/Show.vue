        ...
        <div class="flex h-full flex-1 flex-col gap-4 p-4 sm:gap-6 sm:p-6 bg-[color:color-mix(in_srgb,var(--sispaa-surface)_30%,var(--sispaa-background))]">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-xl font-bold tracking-tight text-[var(--sispaa-text)] sm:text-2xl">{{ empresa.nombre }}</h1>
                        <p v-if="empresa.sector" class="mt-0.5 text-sm opacity-60 text-[var(--sispaa-text)]">{{ empresa.sector }}</p>
                    </div>
                </div>
            ...
            <div class="mx-auto w-full max-w-2xl rounded-2xl p-6 shadow-sm bg-[var(--sispaa-background)]">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <p class="text-sm font-semibold text-[var(--sispaa-text)]">{{ empresa.creator?.name ?? '—' }}</p>
                        <p class="mt-0.5 text-xs opacity-50 text-[var(--sispaa-text)]">{{ formatDate(empresa.created_at) }}</p>
                    </div>
                    <div>
                        <p class="text-xs opacity-50 text-[var(--sispaa-text)]">Última actualización por</p>
                        <p class="text-sm font-semibold text-[var(--sispaa-text)]">{{ empresa.updater?.name ?? '—' }}</p>
                    </div>
                </div>
            </div>
        </div>
