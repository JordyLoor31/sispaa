...
const props = defineProps<{
    ...
    carreras: Catalogo[];
    periodos: Catalogo[];
    filters: { carrera_id?: string; periodo_id?: string; justificada?: string };
...
<template>
    <AppSidebarLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex h-full flex-1 flex-col gap-6 p-6 bg-slate-50/50 dark:bg-slate-900/50">
            <div>
                <h1 class="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Faltas</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Reporte institucional de inasistencias de estudiantes.</p>
            </div>
            ...
            <div class="flex flex-wrap items-end gap-3 bg-white dark:bg-slate-950 p-4 rounded-xl border border-slate-200/80 dark:border-slate-800">
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase mb-1.5">Justificación</label>
            ...
            <div class="rounded-2xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950 overflow-hidden">
                ...
                <div class="flex items-center justify-between border-t border-slate-100 dark:border-slate-800 px-6 py-4">
                    <span class="text-xs text-slate-500">Mostrando {{ faltas.data.length }} de {{ faltas.total }} registros</span>
